...
const ACTIVE_LABEL_ANIM = { animate: { opacity: [0.4, 1, 0.4] }, transition: { duration: 1, repeat: Infinity } };
const SYNTH_ANIM = { initial: { opacity: 0, y: 10 }, animate: { opacity: 1, y: 0 }, exit: { opacity: 0 } };

// ── AgentCard — extracted named component ────────────────────────────────────
const AgentCard = ({ agent, index, isActive, hasOutput, isExpanded, outputText, onToggle }) => {
  ...
  const iconAnim = isActive
    ? { animate: { scale: [1, 1.2, 1], opacity: [0.7, 1, 0.7] }, transition: { duration: 0.8, repeat: Infinity } }
    : {};
  ...
  return (
    <motion.div
      ...
      className={`relative rounded-lg p-2.5 cursor-pointer transition-all duration-300 ${
        hasOutput ? "glass-panel hover:border-opacity-60" : "bg-[#0A0A0F]/60 border border-[#1E293B]"
      }`}
      style={hasOutput ? { borderColor: agent.color + "40" } : {}}
      onClick={() => hasOutput && onToggle(agent.key)}
    >
      {isActive && (
        <motion.div
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-xs font-body font-medium truncate" style={{ color: hasOutput ? agent.color : "#475569" }}>
            {agent.name}
          </div>
      ...
      <AnimatePresence>
        {isExpanded && hasOutput && (
          ...
          >
            <p className="text-[11px] font-body leading-relaxed" style={{ color: "#94A3B8" }}>{outputText}</p>
          </motion.div>
        )}
      </AnimatePresence>
    ...
    <div className="flex flex-col gap-2 h-full" data-testid="agent-chamber">
      <div className="flex items-center gap-2 mb-1">
        <div
          ...
          style={{ background: "#06B6D4", boxShadow: isProcessing ? "0 0 10px #06B6D4, 0 0 20px #06B6D4" : "0 0 6px #06B6D4" }}
        />
        <span className="text-xs font-body text-[#64748B] tracking-[0.2em] uppercase">Agent Chamber</span>
        {isProcessing && (
          <motion.span {...ACTIVE_LABEL_ANIM} className="text-[10px] font-body text-[#06B6D4] ml-auto">
            ACTIVE
          </motion.span>
        )}
      </div>
      ...
      <div className="flex flex-col gap-1.5 flex-1 overflow-y-auto scroll-cyber">
        {AGENTS.map((agent, i) => {
          ...
          const rawOutput = agentOutputs?.[agent.key];
          const hasOutput = Boolean(rawOutput);
          const outputText = typeof rawOutput === "object" ? rawOutput?.output : rawOutput;
      ...
      <AnimatePresence>
        {agentOutputs?.synthesis && (
          <motion.div {...SYNTH_ANIM} className="glass-panel rounded-lg p-3 border-[#3B82F6]/30" data-testid="synthesis-output">
