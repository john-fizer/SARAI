...
const NeuralGraph = ({ nodes, links, selectedNode, onNodeSelect, activeNodeId }) => {
  ...
  // Draw function
  const draw = useCallback((ctx, w, h, ts) => {
  ...
  useEffect(() => {
    ...
    const loop = (ts) => {
      draw(ctx, canvas.width, canvas.height, ts);
  ...
  const handleMouseMove = (e) => {
    ...
    const found = nodesRef.current.find(
      (n) => n.x && n.y && Math.hypot(n.x - mx, n.y - my) < 22
    );
    ...
    <div className="relative w-full h-full" data-testid="neural-graph-container">
      ...
      {nodes.length === 0 && (
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          ...
          >
            <div className="text-6xl mb-6 opacity-20" style={{ filter: "drop-shadow(0 0 20px #06B6D4)" }}>
              ◎
            </div>
          ...
          >
            ...
            <div className="flex gap-1 mt-1 flex-wrap">
              {(tooltip.node.concepts || []).slice(0, 3).map((c) => (
                <span key={c} className="text-[10px] text-[#06B6D4] border border-[#06B6D4]/30 px-1 rounded">
                  {c}
                </span>
              ))}
            </div>
